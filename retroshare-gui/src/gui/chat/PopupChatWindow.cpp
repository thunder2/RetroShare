...
#if QT_VERSION >= QT_VERSION_CHECK (6, 0, 0)
			int x = QRandomGenerator::global()->generate() % 100;
			int y = QRandomGenerator::global()->generate() % 100;
#else
			int x = qrand()%100;
			int y = qrand()%100;
#endif
			this->move(x, y); //avoid to stack multiple popup chat windows on the same position
