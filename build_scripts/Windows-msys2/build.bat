...
call "%~dp0build\build.bat" 64 release webui autologin "CONFIG+=rs_use_native_dialogs"
if errorlevel 1 %cecho% error "Failed to build %SourceName%." & exit /B %ERRORLEVEL%

%cecho% info "Pack %SourceName%"
call "%~dp0build\pack.bat" 64 release webui autologin "CONFIG+=rs_use_native_dialogs"
